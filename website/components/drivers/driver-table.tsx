...
export function DriverTable() {
  ...
  const columns: ColumnDef<DriverRow>[] = [
    ...
    {
      id: "appUser",
      header: "App Kullanici",
      cell: ({ row }) => row.original.user ? `${row.original.user.name} (${row.original.user.email})` : "—",
    },
  ...
  return (
    <div className="space-y-4">
      ...
      />
      {loading ? <p className="text-muted-foreground/60">Yükleniyor...</p> : <DataTable columns={columns} data={data} searchPlaceholder="İsim veya app kullanici ara..." filters={driverFilters} onCellEdit={async (rowIndex, columnId, value) => {
