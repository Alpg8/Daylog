...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const jobLabel = order.cargoNumber ?? order.tripNumber ?? "Yeni iş";
    await recordActivity({
      ...
      entityType: "Order",
      entityId: order.id,
      message: `${order.status} durumunda yeni is olusturuldu`,
      metadata: { cargoNumber: order.cargoNumber, tripNumber: order.tripNumber },
      notifyOps: true,
      notifyDriverUserId: order.driver?.userId ?? null,
      driverTitle: "Yeni İş Atandı",
      driverMessage: `${jobLabel} numaralı iş size atandı.`,
    });
