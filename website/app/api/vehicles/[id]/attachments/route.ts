...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const attachments = await listAttachments("vehicle", params.id);
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const attachment = await uploadAttachment("vehicle", params.id, file, label, expiryDate);
    await recordActivity({
      userId: session.sub,
      role: session.role,
      source: request.headers.get("x-client-source") === "APP" ? "APP" : "WEB",
      action: "UPLOAD_VEHICLE_ATTACHMENT",
      entityType: "Vehicle",
      entityId: params.id,
      message: "Arac dosyasi yuklendi",
