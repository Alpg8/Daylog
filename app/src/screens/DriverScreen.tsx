...
export function DriverScreen({ user, assignedVehicle, activeTaskCount, selectedTask, onTabChange }: DriverScreenProps) {
  return (
    <ScrollView contentContainerStyle={{ paddingBottom: 14 }}>
      <View style={styles.gradientCard}>
        <Text style={styles.gradientCardTitle}>Sofor Sayfasi</Text>
        <Text style={styles.gradientCardText}>{user.name}</Text>
        <Text style={styles.gradientCardText}>{user.email}</Text>
      </View>

      <View style={styles.card}>
        <Text style={styles.cardTitle}>Gorev Ozeti</Text>
        <Text style={styles.cardLine}>Arac: {assignedVehicle}</Text>
        <Text style={styles.cardLine}>Aktif Is: {activeTaskCount}</Text>
      </View>
      ...
      {selectedTask ? (
        ...
        <View style={styles.card}>
          <Text style={styles.emptyText}>Aktif is bulunamadi.</Text>
        </View>
      )}
